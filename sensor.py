...
# DEFINITION DER REGISTER: Ihre vollständige Liste, umgewandelt in ein Python-Dictionary
        ...
        for key, reg_config in DANFOSS_REGISTERS_CONFIG.items():
            try:
                ...
                if not result.isError():
                    ...
                    # Formel: (Rohwert * Skalierung) + Offset
                    scaled_value = (raw_value * scale) + offset
                    data[key] = round(scaled_value, precision)
                else:
                    _LOGGER.warning(f"Error reading register {reg_config['address']}: {result}")
...
class DanfossECLSensor(SensorEntity):
    ...
    def __init__(self, coordinator, key, reg_config):
        ...
        # Setzen von Standard HA Attributen, falls vorhanden
    ...
    def extra_state_attributes(self):
        """Return the state attributes (die zusätzlichen Felder wie max_temp, input_type)."""
        attributes = self._reg_config.copy()
        # Entferne Felder, die bereits als Standard-HA-Attribute verwendet werden
